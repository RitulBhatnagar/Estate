...
import { useGetPropertyForRentByFilterQuery } from '../services/EstateApi'
...
const SearchByRent = () => {
  ...
  const {data, isLoading} = useGetPropertyForRentByFilterQuery({  categoryExternalID, rentFrequency, minPrice, maxPrice,sort});
  ...
  return (
    <>
    {
      isLoading ? <Loader/> : (<Box>
        ...
        <Text fontSize="xl" p = "4" color = "gray" fontFamily = "Raleway">
          Properties for {id}
        </Text>
        <Flex flexWrap = "wrap">
          {
